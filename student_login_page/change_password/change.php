...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($newPassword === $confirmPassword) {
        ...
        // Update the password
        $sql = "UPDATE student_login SET password='$encodedPassword' WHERE username='$rollnumber'";

        if ($conn->query($sql) === TRUE) {
            // Use alert() to display success message
            echo "<script>alert('Password updated successfully.');window.location.href='../student login.html';</script>";
            exit();
        } else {
    ...
    // Use alert() to display error message
    echo "<script>alert('Invalid roll number.');window.location.href='index.php';</script>";
    exit();
}
