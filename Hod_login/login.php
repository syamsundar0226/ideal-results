...
$stmt = $conn->prepare("SELECT sno, name, branch, username, password FROM hod_login WHERE username=? AND password=?");
$stmt->bind_param("ss", $username, $encoded_password); // Bind the encoded password to the query
$stmt->execute();
$stmt->store_result();

if ($stmt->num_rows > 0) {
    $stmt->bind_result($sno, $name, $branch, $username, $password);
